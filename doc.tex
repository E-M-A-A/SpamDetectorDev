...
\newpage
    ...
    Per l'addestramento visto che si parla di elaborare testo in linguaggio naturale con funzioni matematiche dobbiamo prima di tutto trasformare il testo in numeri su cui possiamo applicare gli algoritmi di addestramento del nostro classificatore. Questo passaggio lo abbiamo effettuato con la cosidetta vettorizzazione.
    \newline Nel nostro caso vettorizzare ha voluto dire usare la libreria CountVectorizer per tradurre i commenti in vettori di parole singole per poi sostituire le parole con un intero che rappresenta il numero di occorrenze di quella parola nel commento come mostrato nel seguente esempio.
        \begin{figure}[h]
            \centering
            ‘Hello my name is james’\newline ‘james this is my python notebook’\newline ‘james trying to create a big dataset’\newline ‘james of words to try differnt’\newline ‘features of count vectorizer’\newline
        \end{figure}
        
        \begin{figure}[h]
            \centering
            \includegraphics[width =\textwidth]{immagini/EsempioCounterVectorizer.png}
        \end{figure}
...
\newpage
    ...
    L'agente essendo addestrato ed eseguito in Python mentre il sito di Storytelling è scritto in Java vuol dire che dobbiamo far comunicare due sistemi diversi con metodi di comunicazione che siano interoperabili tra i 2 linguaggi.\newline Il vantaggio di dover sviluppare un sistema del genere è poi avere la possibilità di rendere facilmente l'agente disponibile per altre piattaforme. 

    \section{Prelevamento istanze}
    L'agente all'avvio rimane in ascolto su una socket dove attende la richiesta di analisi dei commenti della piattaforma Storytelling. La piattaforma Storytelling ogni 24 ore e/o al proprio avvio manda un JSON contenente i commenti che sono stati postati il giorno corrente associati ai nomi utente di chi li invia per poi rimanere in attesa sulla socket per ricevere i risulati.\newline L'agente ricevuti i commenti, li inserisce in un dataframe generato tramite la libreria Pandas, li pulisce tramite le stesse techiche che sono state applicate al dataset, li vettoriza tramite il dizionario usato durante il training per poi infine analizzarli.\newline Dopo aver analizzato i commenti, l'agente invia sempre in JSON i nomi utente di coloro che sono stati riconosciuti come spammer e chiude la connessione con la piattaforma per poi rimanere in ascolto per la chiamata successiva.
